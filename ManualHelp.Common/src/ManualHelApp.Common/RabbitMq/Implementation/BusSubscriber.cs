...
using ManualHelp.Common.Messages.Implementation;
...
namespace ManualHelp.Common.RabbitMq.Implementation
{
    public class BusSubscriber : IBusSubscriber
    {
        ...
        public BusSubscriber(IApplicationBuilder app)
        {
            ...
            _retries = 3;//options.Retries >= 0 ? options.Retries : 3;
            _retryInterval = 2;//options.RetryInterval > 0 ? options.RetryInterval : 2;

        }

        public IBusSubscriber SubscribeCommand<TCommand>(string @namespace = null, 
            string queueName = null, Func<TCommand, ManualHelpException, IRejectedEvent> onError = null) 
            where TCommand : ICommand
        {
            var commandHandler1 = _serviceProvider.GetService<ICommandHandler<TCommand>>();
        ...
        public IBusSubscriber SubscribeEvent<TEvent>(string @namespace = null, 
            string queueName = null, Func<TEvent, ManualHelpException, IRejectedEvent> onError = null) 
            where TEvent : IEvent
        {
        ...
        private async Task<Acknowledgement> TryHandleAsync<TMessage>(TMessage message,
            ...
            Func<Task> handle, Func<TMessage, ManualHelpException, IRejectedEvent> onError = null)
        {
            var currentRetry = 0;
            ...
            return await retryPolicy.ExecuteAsync<Acknowledgement>(async () =>
            {
                ...
                using (scope)
                {
                    ...
                    catch (Exception exception)
                    {
                        ...
                        if (exception is ManualHelpException manualHelpException && onError != null)
                        {
                            var rejectedEvent = onError(message, manualHelpException);
            ...
            Func<Task> handle, Func<TMessage, ManualHelpException, IRejectedEvent> onError = null)
        {
            ...
            {
                _logger.LogError(exception, exception.Message);
                if (exception is ManualHelpException dShopException && onError != null)
                {
